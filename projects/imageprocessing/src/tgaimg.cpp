...
void TGAimg::rotateImage()
{
    TGAimg rotatedImg;

    int i = 0;

    for(auto iter = this->image.rbegin(); iter != this->image.rend(); iter++)
    {
        rotatedImg.image.emplace(i, Pixel(iter->second.blue, iter->second.green, iter->second.red));
        i++;
    }

    map<int, Pixel>::iterator rotatedIter = rotatedImg.image.begin();

    for(auto iterForward = this->image.begin(); iterForward != this->image.end(); iterForward++)
    {
        iterForward->second.blue = rotatedIter->second.blue;
        iterForward->second.green = rotatedIter->second.green;
        iterForward->second.red = rotatedIter->second.red;
